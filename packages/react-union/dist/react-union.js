(function (global, factory) {
	...
	(factory((global.ReactUnion = {}),global.ReactDOM,global.React));
...
var invariant$2 = function invariant(pred, msg) {
	if (pred) {
		return;
	}
...
var createElementWithId = function createElementWithId(id, parent) {
...
function justRender(component) {
	...
	ReactDOM__default.render(component, rootElement, resolve);
}
...
var omitNonContextProps = omit_1(['namespace', 'children']);

var WidgetProvider = function (_Component) {
	inherits(WidgetProvider, _Component);

	function WidgetProvider() {
		classCallCheck(this, WidgetProvider);
		return possibleConstructorReturn(this, (WidgetProvider.__proto__ || Object.getPrototypeOf(WidgetProvider)).apply(this, arguments));
	}

	createClass(WidgetProvider, [{
		key: 'getChildContext',
...
var WidgetPortal = function WidgetPortal(_ref) {
	...
	var name = mark.name,
	    ...
	    rest = objectWithoutProperties(mark, ['name', 'container', 'namespace']);

	var resolvedNamespace = namespace || container;

	invariant$2(!WidgetComponent || container, 'Missing attribute "container" for the widget "' + name + '" to be rendered.');

	var widgetProps = { namespace: resolvedNamespace };
	var el = document.getElementById(container);

	warning$2(el, 'HTML container with id "' + container + '" is not found for widget wtih name "' + name + '"');

	return WidgetComponent && el ? ReactDOM.createPortal(React__default.createElement(
		WidgetProvider,
		widgetProps,
		React__default.createElement(WidgetComponent, _extends({}, widgetProps, rest))
	), el) : null;
};

WidgetPortal.propTypes = _extends({}, ConfigShape);

var renderUnionWidget = ifElse_1(isNil_1, noop, map_1(function (config) {
	return React__default.createElement(WidgetPortal, _extends({ key: config.mark.namespace || config.mark.container }, config));
}));

var Widgets = o_1(renderUnionWidget, prop_1('configs'));
...
var validateDomMarks = forEach_1(function (_ref) {
	var name = _ref.name,
	    namespace = _ref.namespace,
	    container = _ref.container;

	invariant$2(name, 'Missing \'name\' in DOM mark.');
	invariant$2(namespace || container, 'Missing required attributes for the widget "' + name + '". ' + 'Fill in "namespace" or "container" in the DOM mark.');
});

var flipContains = flip_1(contains_1);
var getPath = path_1(['path']);
var mapName = map_1(path_1(['name']));
...
var parseDomMarks = o_1(unary_1(JSON.parse), path_1(['innerHTML']));
var getDomMarks = o_1(map_1(parseDomMarks), selectDomMarks);

var getDomMarksByName = o_1(fromPairs_1, map_1(function (x) {
  return [x.name, x];
}));
...
var getComponents = function getComponents(routes, domMarks) {
  ...
  var pairComponentWithMark_ = function pairComponentWithMark_(route) {
    return function (component) {
      return {
        component: component,
        mark: path_1([route.path], marksByName_)
      };
    };
  };

  var loadFoundComponents_ = o_1(map_1(function (route) {
    return routeP(route).then(pairComponentWithMark_(route));
  }), filter_1(o_1(flipContains(paths), getPath)));
...
var Union = function (_Component) {
	...
	function Union() {
		...
		return _ret = (_temp = (_this = possibleConstructorReturn(this, (_ref = Union.__proto__ || Object.getPrototypeOf(Union)).call.apply(_ref, [this].concat(args))), _this), _this.state = {
			configs: []
	...
	createClass(Union, [{
		...
		value: function scan$$1(props) {
			...
			scan(routes, domParent).then(function (configs) {
				onEndScan(configs);
		...
		value: function render() {
			return React__default.createElement(
				...
				null,
				this.props.children,
				React__default.createElement(Widgets, { configs: this.state.configs })
			);
		}
	}]);
	return Union;
...
exports.ConfigShape = ConfigShape;
